...
function getContextualIcyChecklist(text, { profile, occasion, occasionItem, currentStep } = {}) {
  ...
  const stepTitleForMissing = String(currentStep?.title || "").toLowerCase();
  const stepActionForMissing = String(currentStep?.action || "").toLowerCase();
  const stepTextForMissing = `${stepTitleForMissing} ${stepActionForMissing}`;
  const technicalMissingNeeded = /dose|yield|shot|grind|espresso|extraction|machine|puck|milk/.test(stepTextForMissing) || /dose|yield|grams?|shot|time|seconds?|grind|finer|coarser|fast|slow|gush|chok|no flow|few drops|sour|bitter|harsh|dry|watery|thin|puck|channel|spray|milk|foam|steam|temperature|temp|pressure|basket/.test(lower);
  const missing = [];
  if (technicalMissingNeeded) {
  ...
  const stepTitleForMode = String(currentStep?.title || "").toLowerCase();
  const stepActionForMode = String(currentStep?.action || "").toLowerCase();
  const stepAdvisorForMode = String(currentStep?.advisor || "").toLowerCase();
  const stepTextForMode = `${stepTitleForMode} ${stepActionForMode} ${stepAdvisorForMode}`;
  const clearlyTechnical = /dose|yield|grams?|shot|time|seconds?|grind|finer|coarser|fast|slow|gush|chok|no flow|few drops|sour|bitter|harsh|dry|watery|thin|puck|channel|spray|milk|foam|steam|temperature|temp|pressure|basket/.test(lower);
  const intentionStep = /occasion intention|set the occasion|intention|desired feeling|purpose|human purpose/.test(stepTextForMode);
  const stagecraftStep = /script|stagecraft|say|present|serve|guest|resonance|first sip/.test(stepTextForMode);
  const prepStep = /mise|place|readiness|prep|prepare|set up|setup/.test(stepTextForMode);
  const tasteStep = /taste|sip|palate|flavor|sensory/.test(stepTextForMode);
  const stepLocked = (intentionStep || stagecraftStep || prepStep || tasteStep) && !clearlyTechnical;

  let category = stepLocked && intentionStep ? "occasion-intention" : stepLocked && stagecraftStep ? "stagecraft/guest" : stepLocked && prepStep ? "preparation" : stepLocked && tasteStep ? "taste" : "observation";
  if (!stepLocked && /few drops|no flow|barely drip|barely dripping|chok|nothing came out|stalled/.test(lower)) category = "choke/no-flow";
  ...
  if (category === "occasion-intention") checklist = [
    ...
    "What feeling should it create: soft, steady, gentle, cared for, playful, impressive, or calm?",
    "Who is receiving the moment, and what do they need right now?",
    "Should ICY keep this quiet and human-centered before moving into technique?"
  ];
  else if (category === "preparation") checklist = [
    "What must be ready before the machine work begins?",
    "Are drinkware, water, milk, beans, towel, tools, and serving path in place?",
    "What could interrupt the moment if it is not handled now?",
    "Should ICY save this as readiness context or a recovery concern?"
  ];
  else if (category === "taste") checklist = [
    "What did you notice first: sweetness, acidity, body, texture, finish, or balance?",
    "Did the cup match the intended Occasion feeling?",
    "Would you serve it again in this moment?",
    "Should ICY save this as a preference, adjustment, or guest-resonance note?"
  ];
...
function buildOccasionAwareAdvisorReply(text, { occasionItem, currentStep, stepNumber, totalSteps, profile, occasion, changedFields, routing } = {}) {
  ...
  const missingLine = stepLocked
    ? " I am keeping this anchored to the active step and will not ask for machine variables unless you raise a technical issue."
  ...
  const stepTextForMode = `${stepTitleForMode} ${stepActionForMode} ${stepAdvisorForMode}`;
  const clearlyTechnical = /dose|yield|grams?|shot|time|seconds?|grind|finer|coarser|fast|slow|gush|chok|no flow|few drops|sour|bitter|harsh|dry|watery|thin|puck|channel|spray|milk|foam|steam|temperature|temp|pressure|basket/.test(lower);
  const stepLocked = /occasion intention|set the occasion|intention|desired feeling|purpose|human purpose|script|stagecraft|say|present|serve|guest|resonance|first sip|mise|place|readiness|prep|prepare|set up|setup|taste|sip|palate|flavor|sensory/.test(stepTextForMode) && !clearlyTechnical;

  if (ctx.category === "occasion-intention") {
    guidance = `This is an Occasion intention step. I am staying with purpose before technique. Say the purpose out loud, choose the desired feeling, and let that become the anchor for the rest of the Occasion. A good capture would sound like: “This is a quiet table moment. I want it to feel soft, steady, gentle, and cared for.”`;
  } else if (ctx.category === "preparation") {
    guidance = `This is a preparation/readiness step. Stay with the setup and remove friction before the cup begins. ICY should protect the moment here, not jump ahead into extraction unless you report a technical issue.`;
  } else if (ctx.category === "taste") {
    guidance = `This is a taste and palate step. Describe what landed first and whether it matched the intended Occasion. ICY should capture preference and guest resonance before suggesting a technical adjustment.`;
